...
func _build_forest(art: RegionArt) -> void:
	for tree: Dictionary in layout.trees:
		...
		var prop: Sprite2D = art.sprite(generated, RegionArt.SWAMP if wet else RegionArt.PLAINS,
			region, Vector2(tree.cell * 8), Vector2(5, 5))
	...
	for detail: Dictionary in layout.details:
		...
		var region := Rect2i(detail.kind * 16, 176, 16, 16) if detail.wet else Rect2i(72 + detail.kind * 16, 48, 16, 16)
		art.sprite(generated, sheet, region, Vector2(detail.cell * 8))
	...
	for index: int in range(RegionLayout.CAMPS.size()):
		...
		art.sprite(generated, RegionArt.PLAINS, Rect2i(0, 40, 56, 24), camp + Vector2(30, -48), Vector2(38, 8))
		art.sprite(generated, RegionArt.PLAINS, Rect2i(0, 0, 16, 32), camp + Vector2(-40, -8), Vector2(6, 5))
