...
The main scene now builds a 304 × 240 tile region (2,432 × 1,920 pixels). Rekala is fixed: nine named residents, workshops, a well and square, farm fields and an animated windmill. Three bridges connect the woodland routes across the river. Nine forest clearings and four town approaches contain 35 goblins/orcs. The closest encounter is along the west road outside the fence. Monsters retain their existing restart behavior.
...
- `region_art.gd`: cached atlas textures and prop footprints defined in sprite-local pixels. Art and collision share the bottom-centre of that footprint as their origin. Trees sort at their feet and collide at their trunks; the windmill uses a polygon matching its base. Flowers and low ground details draw below actors.
...
Select the World node in the editor to change `Preview Seed` and click **Rebuild region preview**. Editor previews show static NPC and enemy sprites without AI, physics, provider calls or access to the player save. Runtime actors spawn when the game runs; enemies are named `Enemy00`, `Enemy01`, etc. beneath `World/Region` in the Remote tree. The runtime seed comes from the save, not the preview setting. New landmarks should reserve their footprint before the tree pass and connect to the road graph. Keep saved landmark positions stable when extending a released world.
...
The second check requires the local art. It instantiates the real world with an isolated temporary save and disabled provider endpoints. It checks nine NPCs and 35 enemies, walkable bridges, prop footprints, houses/water, unobstructed NPC spawns, intact swamp crops, and a nearby enemy's chase decision. It never writes the player's save.
