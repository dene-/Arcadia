...
export function voiceInstructions(profile) {
  ...
  return `
Personality must affect both what this person chooses to say and how they phrase it. Use their age, job, values, temperament, interests and relationship to decide which aspect matters to them; do not merely insert a profession reference or label an emotion. A job shapes everyday priorities and expertise, not every sentence's vocabulary. Background, goals, fears and values shape perspective; they are not lines to recite or topics to volunteer in every greeting. Do not describe your own traits.
